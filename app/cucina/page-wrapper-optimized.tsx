...
export default function CucinaPageOptimized() {
  ...
  const {
    ...
    stationType: StationType.CUCINA,
  ...
  const loadOrders = useCallback(async () => {
    try {
      // Check cache first
      const cachedOrders = getCachedData<Ordinazione[]>('orders:active:cucina');
      if (cachedOrders && cachedOrders.length > 0) {
        console.log('[Cucina] Using cached orders:', cachedOrders.length);
      ...
      console.log('[Cucina] All orders received:', serializedData.length);
      ...
      console.log('[Cucina] ALL UNIQUE DESTINATIONS IN DATABASE:', Array.from(allDestinations));
      ...
      if (itemsWithoutDestination.length > 0) {
        console.log(`[Cucina] WARNING: ${itemsWithoutDestination.length} items without destination!`);
      ...
      serializedData.forEach((ord: any, index: number) => {
        console.log(`\n=== DEBUG ORDER ${index + 1} ===`);
        console.log(`[Cucina] Order ID: ${ord.id}`);
        console.log(`[Cucina] Order Type: ${ord.tipo}`);
        console.log(`[Cucina] Order Stato: ${ord.stato}`);
        console.log(`[Cucina] Number of righe: ${ord.righe.length}`);
        ...
        // Check if any riga has CUCINA destination
        const hasCucinaItems = ord.righe.some((item: any) => item.postazione === 'CUCINA');
        console.log(`[Cucina] Has CUCINA items: ${hasCucinaItems}`);
        
        // Show exactly which items match CUCINA
        const cucinaItems = ord.righe.filter((item: any) => item.postazione === 'CUCINA');
        console.log(`[Cucina] CUCINA items count: ${cucinaItems.length}`);
        cucinaItems.forEach((item: any, itemIndex: number) => {
          console.log(`  CUCINA Item ${itemIndex + 1}: ${item.prodotto?.nome} (ID: ${item.id})`);
        });
        
        // Test the exact filter condition
        const passesFilter = ord.righe.some((item: any) => item.postazione === 'CUCINA');
        console.log(`[Cucina] Passes filter: ${passesFilter}`);
        console.log(`=== END DEBUG ORDER ${index + 1} ===\n`);
      });

      // Transform and filter for CUCINA items only
      const cucinaOrders = serializedData
        .filter((ord: any) => ord.righe.some((item: any) => item.postazione === 'CUCINA'))
        .map((ord: any) => ({
          id: ord.id,
          ...
          items: ord.righe
            .filter((item: any) => item.postazione === 'CUCINA')
            .map((item: any) => ({
              id: item.id,
          ...
          stato: ord.stato,
          hasKitchenItems: ord.righe.some((item: any) => item.postazione === 'PREPARA'),
          cameriere: ord.cameriere?.nome,
          note: ord.note
        }))
        .filter((ord: any) => ord.items.length > 0);

      console.log('[Cucina] Filtered CUCINA orders:', cucinaOrders.length);

      setOrders(cucinaOrders);
      setIsLoading(false);
  ...
  const selectOrder = useCallback((order: Ordinazione) => {
    console.log('[Cucina] Selecting order:', order.id);
  ...
  const autoSelectFirstOrder = useCallback(async (newOrder: Ordinazione) => {
    console.log('[Cucina] Auto-selecting first order:', newOrder.id);
    ...
    if (firstPendingItem) {
      console.log('[Cucina] Auto-starting first item:', firstPendingItem.id);
      ...
      try {
        ...
        console.log('[Cucina] Auto-starting will be handled after component mount');
      } catch (error) {
        console.error('[Cucina] Error auto-starting item:', error);
      }
    }
  ...
  const handleNewOrder = useCallback((data: any) => {
    console.log('[Cucina] New order received:', data);
    ...
    const newOrder: Ordinazione = {
      ...
      items: data.items
        .filter((item: any) => item.destination === 'CUCINA')
        .map((item: any) => ({
          id: item.id,
      ...
      stato: 'ORDINATO',
      hasKitchenItems: data.items.some((item: any) => item.destination === 'PREPARA'),
  ...
  const handleItemUpdate = useCallback((data: any) => {
    console.log('[Cucina] Item update:', data);
    
    setOrders(prev => prev.map((order: Ordinazione) => {
      if (order.id === data.orderId) {
        ...
        const allReady = updatedOrder.items.every((item: OrderItem) => item.stato === 'PRONTO');
        if (allReady) {
  ...
  const handleOrderCancelled = useCallback((data: any) => {
    setOrders(prev => prev.filter((order: Ordinazione) => order.id !== data.orderId));
  ...
  const getOrdersByStatus = useCallback((status: string[]) => {
    return orders.filter((order: Ordinazione) => 
      order.items.some((item: OrderItem) => status.includes(item.stato))
    );
  }, [orders]);
  ...
  // Auto-start first item when a new order is selected - only for CUCINA items
  useEffect(() => {
    if (selectedOrder && selectedOrder.items.length > 0) {
      const firstPendingItem = selectedOrder.items.find((item: OrderItem) => 
        item.stato === 'INSERITO' && item.postazione === 'CUCINA'
      );
      if (firstPendingItem) {
        console.log('[Cucina] Auto-starting first CUCINA item for selected order:', firstPendingItem.id);
  ...
  }, [selectedOrder?.id, selectedOrder?.items?.find((item: OrderItem) => item.stato === 'INSERITO' && item.postazione === 'CUCINA')?.id]);
  ...
  useEffect(() => {
    ...
    eventQueue.forEach(({ event, data, timestamp }) => {
      console.log(`[Cucina] Processing event: ${event}`, data);
      
      switch (event) {
        case 'order:new':
          if (data.items?.some((item: any) => item.destination === 'CUCINA')) {
            handleNewOrder(data);
          }
          break;
          
        case 'order:item:update':
          if (data.destination === 'CUCINA') {
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-semibold text-foreground">Postazione Cucina</h1>
        ...
        <div className="space-y-4 overflow-y-auto scrollbar-hide">
          ...
          {orders.length === 0 ? (
            <div className="text-center py-12 bg-card rounded-lg border border-border">
              ...
              <p className="text-muted-foreground">Nessun ordine in coda per CUCINA</p>
...
function OrderListCard({
  ...
  const getOrderStatusBadge = () => {
    const hasInProgress = order.items.some((item: OrderItem) => item.stato === 'IN_LAVORAZIONE');
    const hasReady = order.items.some((item: OrderItem) => item.stato === 'PRONTO');
    const allReady = order.items.every((item: OrderItem) => item.stato === 'PRONTO');
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border overflow-hidden">
      ...
      <div className="p-6">
        <h4 className="font-medium mb-4 flex items-center gap-2">
          ...
          Prodotti da Cucinare ({order.items.length})
        </h4>
            ...
            <div key={item.id} className={`flex items-center justify-between p-4 rounded-lg ${
              ...
              <div className="flex-1">
                <div className={`font-medium ${
                  item.postazione !== 'CUCINA' ? 'text-gray-500' : ''
                }`}>
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-600">
          <div className="flex justify-center">
            {order.items.every((item: OrderItem) => item.stato === 'PRONTO') ? (
              ...
              <button
                onClick={async () => {
                  ...
                  const currentIndex = orders.findIndex((o: Ordinazione) => o.id === order.id);
                  const nextOrders = orders.filter((o: Ordinazione, index: number) => 
                ...
                disabled={order.items.some((item: OrderItem) => processingItems.has(item.id))}
              ...
              >
                {order.items.some((item: OrderItem) => processingItems.has(item.id)) ? (
                  <>
        ...
        {order.hasKitchenItems && (
          <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
            <div className="flex items-center gap-2 text-orange-800 text-sm">
              ...
              <span className="font-medium">Questo ordine include anche elementi per il bar</span>
            </div>
          </div>
        )}
